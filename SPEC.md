...
6. Recording against a *real agent* and `L2` (llm_infer) are blocked on an API key — nothing in this environment has one configured. Everything else in weeks 1-3 that doesn't require a live model call is implemented and tested, including `adapters/agentdojo.py` + `trace/record.py`: AgentDojo ships a `GroundTruthPipeline` that replays a task's known-correct tool-call sequence with zero model calls, which is enough to build and validate the full recording pipeline (schema conversion, content-addressed span ids, credential hashing, JSONL round-trip) against real tasks. All 97 AgentDojo v1 user tasks across all 4 suites convert cleanly end-to-end. What this *can't* produce is `ASR_t`/`BTC` data — the ground-truth agent never deviates from the correct path, so there's no attack behavior to measure — that still needs a real pipeline and a key.
